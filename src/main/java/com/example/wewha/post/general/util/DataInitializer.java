...
public class DataInitializer {
    ...
    public CommandLineRunner initData() {
        return args -> {
            // --- 1. 유저 데이터 생성 ---
            User user1 = userRepository.save(User.builder().email("user1@example.com").nickname("여행가").build());
            User user2 = userRepository.save(User.builder().email("user2@example.com").nickname("맛집탐방러").build());
            User user3 = userRepository.save(User.builder().email("user3@example.com").nickname("개발자").build());
